#!/usr/bin/env python3
"""
`16_external_validation_and_generalisation/05_validate_framework_on_independent_dataset.py`
...
breast-cancer dataset (`16_external_validation_and_generalisation/01_acquire_independent_spatial_dataset.py`) -- the registered
`q1_framework_generalisation` claim, the strongest single test of Q1.
See
src/xenium_tcr_ecology/validation/framework_generalisation.py's module
docstring.

Primary output: reports/validation/framework_generalisation.pdf
"""
...
from xenium_tcr_ecology.validation.framework_generalisation import (
    build_framework_generalisation_test,
)
...
def render_calibration_plot(calibration_summary: dict, output_path) -> None:
    ...
    ax.set_title("Framework calibration on the independent Xenium breast-cancer dataset")
    ax.legend()
...
def main() -> int:
    ...
    logger = JsonRunLogger(
        ...
        script_name="05_validate_framework_on_independent_dataset",
        project_root=project_root,
        phase="16_external_validation_and_generalisation",
    )

    try:
        summary = build_framework_generalisation_test(project_root)
        report_path = project_root / "reports" / "validation" / "framework_generalisation.pdf"
